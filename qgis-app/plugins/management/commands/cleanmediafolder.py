...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for static_dir in os.listdir(static_root):
            ...
            if os.path.isdir(static_path) and os.path.exists(
                os.path.join(media_root, static_dir)
            ):
                confirm = input(
                    f"Are you sure you want to delete '{static_dir}' from the media folder? (yes/no): "
                )
                ...
                if confirm.lower() == "yes":
                    try:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"Deleted {static_dir} from media folder."
                            )
                        )
                    except Exception as e:
                        self.stderr.write(
                            self.style.ERROR(f"Error deleting {static_dir}: {str(e)}")
                        )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"Skipped deletion of {static_dir}.")
                    )
