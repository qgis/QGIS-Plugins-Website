...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        versions = PluginVersion.objects.filter(created_on__lt="2014-12-31").exclude(
            package__icontains="2014/"
        )
        self.stdout.write(
            self.style.NOTICE(f"{versions.count()} packages will be organized.")
        )
        ...
        for version in versions:
            ...
            if os.path.exists(old_path):
                ...
                if not os.path.exists(new_path):
                    ...
                    version.package.name = os.path.relpath(
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Plugin version id {version.pk} ignored: {new_path} already exists."
                        )
                    )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"Plugin version id {version.pk} ignored: {old_path} is not found."
                    )
                )
