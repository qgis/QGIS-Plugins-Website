    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                new_version.save()
                msg = _(
                    "The Plugin has been successfully uploaded. Security and quality checks are now running asynchronously. A Qt6 compliance check will be launched. You'll receive the results by email."
                )
                messages.success(request, msg, fail_silently=True)
