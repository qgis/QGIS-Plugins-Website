...
class TestPluginTask(TestCase):
    ...
    def test_update_qgis_versions(
        self, mock_get_qgis_versions, mock_create, mock_first
    ):
    ...
    @patch.object(SitePreference.objects, "create")
    @patch("plugins.tasks.update_qgis_versions.get_qgis_versions")
    def test_update_qgis_versions_no_site_preference(
        self, mock_get_qgis_versions, mock_create, mock_first
    ):
    ...
    def test_generate_plugins_xml(self, mock_open, mock_mkdir, mock_exists, mock_get):
        ...
        mock_open.assert_any_call(
            os.path.join(expected_folder_path, "plugins_3.24.xml"), "w+"
        )
        mock_open.assert_any_call(
            os.path.join(expected_folder_path, "plugins_3.25.xml"), "w+"
        )
    ...
    @patch("os.mkdir")
    @patch("builtins.open", new_callable=MagicMock)
    def test_generate_plugins_xml_with_custom_site(
