...
class SetupMixin:
    ...
    def setUp(self) -> None:
        ...
        # A verified contact email is an acceptance criterion for appearing in
        # the review lists, so confirm both plugins' emails by default. Tests
        # that need an unconfirmed email override this explicitly.
        for plugin in (self.plugin_1, self.plugin_2):
            self.confirm_plugin_email(plugin)

    @staticmethod
    def confirm_plugin_email(plugin):
        confirmation, _ = PluginEmailConfirmation.create_for_email(
            plugin.email, [plugin]
        )
    ...
    @staticmethod
    def unconfirm_plugin_email(plugin):
        """Drop every confirmation for the plugin's email so it reads as
        unverified (as it would be before the author clicks the link)."""
        PluginEmailConfirmation.objects.filter(email=plugin.email).delete()
...
class TestFeedbackNotify(SetupMixin, TestCase):
    ...
    def test_version_feedback_notify_sent(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, f"Plugin {self.plugin_1} feedback notification."
        )
        self.assertIn(
            (
        ...
        self.assertEqual(mail.outbox[0].recipients(), ["email@example.com"])
    ...
    def test_add_recipient_in_email_notification(self):
        ...
        new_recipient = User.objects.create(
            username="new-recipient", email="new@example.com"
        )
        self.plugin_1.owners.add(new_recipient)
        self.assertListEqual(list(self.plugin_1.editors), [new_recipient, self.creator])
        with self.assertLogs(level="DEBUG"):
            version_feedback_notify(self.version_1, self.staff)
        self.assertEqual(
            mail.outbox[0].recipients(), ["new@example.com", "email@example.com"]
        )
...
class TestPluginFeedbackCompletedList(SetupMixin, TestCase):
    ...
    def test_version_feedback_resolved_notify(self):
        ...
        self.assertEqual(mail.outbox[0].recipients(), ["staff@staff.it"])
    ...
    def test_staff_should_see_plugin_feedback_completed(self):
        ...
        self.assertEqual(list(response.context["object_list"]), [self.plugin_1])
    ...
    def test_unconfirmed_email_plugin_hidden_from_feedback_completed_list(self):
        self.client.force_login(user=self.staff)
        response = self.client.get(self.url)
        self.assertIn(self.plugin_1, response.context["object_list"])

        self.unconfirm_plugin_email(self.plugin_1)
        response = self.client.get(self.url)
...
class TestPluginFeedbackReceivedList(SetupMixin, TestCase):
    ...
    def test_plugin_with_older_approved_version_shows_in_feedback_received_list(self):
        ...
        PluginVersionFeedback.objects.create(
            version=newer_version, reviewer=self.staff, task="feedback on newer version"
        )
    ...
    def test_plugin_with_latest_version_approved_does_not_show_in_feedback_received_list(
        self,
    ):
    ...
    def test_unconfirmed_email_plugin_hidden_from_feedback_received_list(self):
        self.client.force_login(user=self.staff)
        response = self.client.get(self.url)
        self.assertIn(self.plugin_1, response.context["object_list"])
...
class TestPluginFeedbackPendingList(SetupMixin, TestCase):
    ...
    def test_staff_should_see_plugin_feedback_pending_list(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "plugins/plugin_list.html")
        self.assertEqual(list(response.context["object_list"]), [self.plugin_2])
        self.assertContains(response, "test plugin 2")
    ...
    def test_plugin_with_latest_version_approved_does_not_show_in_feedback_pending_list(
        self,
    ):
    ...
    def test_unconfirmed_email_plugin_hidden_from_feedback_pending_list(self):
        self.client.force_login(user=self.staff)
        response = self.client.get(self.url)
        self.assertIn(self.plugin_2, response.context["object_list"])

        self.unconfirm_plugin_email(self.plugin_2)
        response = self.client.get(self.url)
        self.assertNotIn(self.plugin_2, response.context["object_list"])
...
class TestCreateVersionFeedback(SetupMixin, TestCase):
    def setUp(self) -> None:
        ...
        self.new_user = User.objects.create(username="new-user", is_staff=False)
    ...
    def test_post_create_single_task_feedback(self):
        ...
        response = self.client.post(self.url, data={"feedback": "single line feedback"})
        self.assertEqual(response.status_code, 302)
        feedbacks = PluginVersionFeedback.objects.filter(version=self.version_2).all()
    ...
    def test_post_create_multiple_task_feedback(self):
        ...
        response = self.client.post(
            self.url, data={"feedback": "- [ ] task one\n - [ ] task two"}
        )
        self.assertEqual(response.status_code, 302)
        feedbacks = PluginVersionFeedback.objects.filter(version=self.version_2).all()
    ...
    def test_post_create_invalid_bullet_point_1(self):
        self.client.force_login(self.staff)
        self.client.post(
            self.url, data={"feedback": "-[ ] invalid bullet point \n -[ ] invalid two"}
        )
        feedbacks = PluginVersionFeedback.objects.filter(version=self.version_2).all()
        self.assertEqual(len(feedbacks), 1)
        self.assertEqual(
            feedbacks[0].task, "-[ ] invalid bullet point \n -[ ] invalid two"
        )
...
class TestDeleteVersionFeedback(SetupMixin, TestCase):
    ...
    def test_only_the_reviewer_can_delete_a_feedback(self):
        self.client.force_login(user=self.creator)
        response = self.client.post(self.url, data={"status_feedback": "deleted"})
        self.assertEqual(response.status_code, 200)
...
class TestUpdateVersionFeedback(SetupMixin, TestCase):
    ...
    def test_staff_and_editor_can_update_feedback(self):
        ...
        self.client.force_login(user=self.creator)
        response = self.client.post(
            self.url, data={"completed_tasks": [feedbacks[0].id]}
        )
    ...
    def test_non_staff_and_non_editor_cannot_update_feedback(self):
        ...
        self.client.post(self.url, data={"status_feedback": [feedback.id]})
        feedback = self.version_1.feedback.first()
