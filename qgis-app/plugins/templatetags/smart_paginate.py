...
def smart_paginate(context, window=DEFAULT_WINDOW, hashtag=""):
    ...
    try:
        ...
        # ABP: per_page
        if paginator.count > DEFAULT_PAGINATION:
            per_page_list = range(0, paginator.count + 1, DEFAULT_PAGINATION)
            per_page_list = per_page_list[1:]
            per_page_list.append(paginator.count)
        else:
            per_page_list = []
        to_return = {
